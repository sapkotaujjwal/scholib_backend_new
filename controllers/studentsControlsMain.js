...
<body>
    ...
    // Push the student into the randomly selected section
    allSections[rSectionIndex].students.push(updatedDoc);

    // Save the updated course object back to the database
    await course.save();

    updatedDoc.course.section = allSections[rSectionIndex]._id.toString();

    if (!updatedDoc.course.section) {
      throw new Error("Section not added");
    }

    school.students.push(updatedDoc);
...
async function startBusService(req, res, next) {
  try {
    ...
    // I want this part to be checked from a query but okay for now

    const course = await Course.findOne({ schoolCode });
    const student = findStudent(course, classId, groupId, sectionId, _id);
    if (student.bus.length > 0 && !student.bus[0].end) {
      throw new Error("Bus service is already active");
    }
    ...
    const updatedCourse = await Course.findOneAndUpdate(
      ...
      {
        $push: {
          "course.$[class].groups.$[group].sections.$[section].students.$[student].bus":
            {
              $each: [objToAdd],
              $position: 0,
            },
...
async function endBusService(req, res, next) {
  try {
    ...
    const student = findStudent(course, classId, groupId, sectionId, _id);
    if (student.bus.length === 0 || student.bus[0].end) {
      throw new Error("Bus service is already inactive");
    }
    ...
    await Course.findOneAndUpdate(
      ...
      {
        $set: {
          "course.$[class].groups.$[group].sections.$[section].students.$[student].bus.0.end":
            getDate().fullDate,
        },
      },
      {
    ...
    await Course.findOneAndUpdate(
      {
        ...
        "course.groups.sections._id": sectionId,
        "course.groups.sections.students._id": _id,
        "course.groups.sections.students.fine": { $exists: true },
      },
      {
        $push: {
          "course.$[class].groups.$[group].sections.$[section].students.$[student].fine":
            {
              $each: [objToAdd],
              $position: 0,
            },
    ...
    await Course.findOneAndUpdate(
      {
        ...
        "course.groups.sections._id": sectionId,
        "course.groups.sections.students._id": _id,
        "course.groups.sections.students.discount": { $exists: true },
      },
      {
        $push: {
          "course.$[class].groups.$[group].sections.$[section].students.$[student].discount":
            {
              $each: [objToAdd],
              $position: 0,
            },
...
async function payFees(req, res, next) {
  try {
    ...
    const dateAndTime = await getCurrentNepaliDate();
